...
const ADB = `${process.env.ANDROID_HOME || '/home/vscode/android-sdk'}/platform-tools/adb`;
...
app.post('/api/tap', (req, res) => {
  const { x, y } = req.body;
  exec(`${ADB} -s ${DEVICE} shell input tap ${Math.round(x)} ${Math.round(y)}`, () => res.json({ ok: true }));
});

app.post('/api/swipe', (req, res) => {
  const { x1, y1, x2, y2, duration = 200 } = req.body;
  exec(`${ADB} -s ${DEVICE} shell input swipe ${x1} ${y1} ${x2} ${y2} ${duration}`, () => res.json({ ok: true }));
});
...
app.post('/api/key', (req, res) => {
  ...
  exec(`${ADB} -s ${DEVICE} shell input keyevent ${keycode}`, () => res.json({ ok: true }));
});

app.post('/api/text', (req, res) => {
  const { text } = req.body;
  exec(`${ADB} -s ${DEVICE} shell input text "${text.replace(/"/g, '\\"')}"`, () => res.json({ ok: true }));
});

app.post('/api/launch', (req, res) => {
  const { pkg } = req.body;
  exec(`${ADB} -s ${DEVICE} shell monkey -p ${pkg} -c android.intent.category.LAUNCHER 1`, () => res.json({ ok: true }));
});
...
app.post('/api/adb', (req, res) => {
  ...
  exec(`${ADB} -s ${DEVICE} shell ${command}`, { timeout: 5000 }, (err, stdout, stderr) => {
...
wss.on('connection', (ws) => {
  ...
  const checkReady = setInterval(() => {
    exec(`${ADB} -s ${DEVICE} shell echo ok`, (err, stdout) => {
...
function startStream(ws) {
  ...
  proc.on('exit', (code) => {
    ...
    if (ws.readyState === ws.OPEN) setTimeout(() => startStream(ws), 1000);
  });
}
